...
//	blockInfo *root = get_file_block( table_name, IndexFile, 0);
//	blockInfo *node = get_file_block( table_name, IndexFile, root->blockNum);
//	if (node == NULL)
//		return -3;
...
//			}
//		}
//		node = get_file_block( table_name, IndexFile, blocknum);
//		if (node == NULL)
//			return -3;
//		info = node->cBlock;
//	}
...
//	else if (inform.type == Char_n)
//		length = StrLen;
//	blockInfo *node = get_file_block( table_name, IndexFile, blocknum);
//	if (node == NULL) {
//		inform.offset = 0;
//		return -3;
//	}
...
//	blockInfo *node = get_file_block( table_name, IndexFile, 0);
//	if (node == NULL)
//		return -3;
//	string left;
//	string info = node->cBlock;
...
//		node = get_file_block( table_name, IndexFile, StrToI(left));
//		info = node->cBlock;
//	}
//	return StrToI(left);
//}
...
//		root = get_file_block( table_name, IndexFile, blockNum);
//		info = root->cBlock;
//	}
//	return parent;
//}
//
//void IndexManager::search_many(const std::string& database, const std::string& table_name, int& start, int& end, int type, struct index_info& inform) {
//	int length, next;
//	if (inform.type == Int)
//		length = IntLen;
//	else if (inform.type == Float)
...
//		length = StrLen;
//	int blocknum = search_leaf(database, table_name, inform);
//	blockInfo *leaf = get_file_block( table_name, IndexFile, blocknum);
...
//				string tmp = get_file_block( table_name, IndexFile, start)->cBlock;
...
//				string tmp = get_file_block( table_name, IndexFile, end)->cBlock;
//				inform.offset = getoffset(database, table_name, inform, type, end);
//			}
//			else {
//				end = leaf->blockNum;
...
//	}
//	blockInfo *Node = get_file_block( table_name, IndexFile, n);
//	string info = Node->cBlock;
//	string originN = info;
...
//		blockInfo *child = get_file_block( table_name, IndexFile, StrToI(tmpN.substr(5, 3)));
...
//				blockInfo *N1 = get_file_block( table_name, IndexFile, n1);
...
//						blockInfo* Parent = get_file_block( table_name, IndexFile, parent);
...
//	blockInfo *node = get_file_block( table_name, IndexFile, Node);
//	string info = node->cBlock;
//	if (StrToI(info.substr(1, ValueLen)) < N - 1)
//		insert_leaf(database, table_name, inform, Node);
//	else {  // divide T - L1
//		blockInfo *L1 = get_file_block(table_name, 0, IndexFile);
//		int l1 = L1->blockNum;
//		blockInfo *T = get_file_block(table_name, 0, IndexFile);
...
//	blockInfo *leaf = get_file_block( table_name, IndexFile, Node);
//	string info = leaf->cBlock;
//	int count = StrToI(info.substr(1, ValueLen));
...
//		blockInfo *R = get_file_block(table_name, 0, IndexFile);
//		int r = R->blockNum;
...
//		return;
//	}
//	blockInfo *N0 = get_file_block( table_name, IndexFile, Node);
//	int p = findParent(database, table_name, inform, N0->blockNum);
//	blockInfo *P = get_file_block( table_name, IndexFile, p);
...
//			}
//		}
//		blockInfo *P1 = get_file_block(table_name, 0, IndexFile);
//		int p1 = P1->blockNum;
...
void IndexManager::search_many(const std::string& database, const std::string& table_name, int& start, int& end, int type, struct index_info& inform, deque<int>& container) {
	int length, next;
	if (inform.type == Int)
		length = IntLen;
	else if (inform.type == Float)
...
void IndexManager::get_index(const std::string& database, const std::string& table_name, int begin, int end, int type, struct index_info& inform, deque<int>& container) {
	int length, count, start, compare;
	if (inform.type == Int)
		length = IntLen;
	else if (inform.type == Float)
...
void IndexManager::delete_entry(const string& database, const string& table_name, struct index_info& inform, int n, const string& K, int nod) {
	...
	write(Node, tmpN);
	if (Node->blockNum == 0 && originN.substr(1, 4) == "0001") { // root has only one child
		blockInfo *child = get_my_file_block(database, table_name, IndexFile, StrToI(tmpN.substr(5, 3)));
		Node->blockNum = child->blockNum;
		child->blockNum = 0;
		writeRootBlock(database, table_name, child);
		deleteBlock(database, table_name, Node);
...
void IndexManager::insert_one(const std::string& database, const std::string& table_name, struct index_info& inform) {
	...
	if (StrToI(info.substr(1, ValueLen)) < N - 1)
		...
		blockInfo *L1 = get_my_new_block(database, table_name);
		int l1 = L1->blockNum;
		blockInfo *T = get_my_new_block(database, table_name);
		int t = T->blockNum; // tmp backup of leaf node;
		T->cBlock = (char *)info.c_str();
		string pn;
		if (info[node->charNum - 1] == '#')
...
void IndexManager::insert_parent(const std::string& database, const std::string& table_name, struct index_info& inform, int Node, const string& K1, int N1) {
	...
	if (inform.type == Int)
		...
		length = StrLen;
	if (root->blockNum == Node) { // Node == 0
		blockInfo *R = get_my_new_block(database, table_name);
		int r = R->blockNum;
		string info = "?0001" + IToStr(r, ChildLen) + K1 + IToStr(N1, ChildLen);
		R->blockNum = Node;
		root->blockNum = r;
