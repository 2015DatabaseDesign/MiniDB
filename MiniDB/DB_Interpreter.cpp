...
sql_node DB_interpreter::Interpreter(string statement) {
	...
	if (regex_match(statement, cap_str, command_split)) {
		...
		case SHOW: return show_clause(statement); break;
		case DESCRIBE_: return describe_clause(statement); break;
		default:
...
sql_node DB_interpreter::select_clause(string SQL) {
	...
	*/
	...
	const regex select_split1("^\\s*select\\s+([\\s\\S,]+)\\s+from\\s+([^\\s]+)(?:\\s+where\\s+(\\S.*\\S))\\s*;\\s*"); // with 'where'
...
sql_node DB_interpreter::show_clause(string SQL) {
	const regex show_split("\\s*show\\s+tables\\s*;\\s*");
	if (regex_match(SQL, show_split))
		return sql_node(SHOW_TABLES);
	else
		throw IllegalCommand(UnrecognizedCMD, SQL);
}

sql_node DB_interpreter::describe_clause(string SQL) {
	const regex describe_split("\\s*describe\\s+(\\w+)\\s*;\\s*");
	smatch describe_match;
	byte type = DESCRIBE, func = 0;
	size_t fa_len = 1;
	static char fa_chr[MAXLENGTH];
	static char *fa[1];
	if (regex_match(SQL, describe_match, describe_split)) {
		strcpy(fa_chr, string(describe_match[1]).data());
		fa[0] = fa_chr;
		return sql_node(type, func, fa_len, fa);
	}
	else
